...
class SemuaAgenda extends StatelessWidget {
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void loadData() async {
    String tmpapp = await http.read(Uri.parse('https://simtaru.kaltimprov.go.id/api/events'));
    List<dynamic> tmpDataapp = jsonDecode(tmpapp).toList();
    setState(() {
      for (var element in tmpDataapp) {
        listIdApps.add(element['id']??'');
        listJudulApps.add(element['tema']??'');
                                  ...
                                  onTap: () {
                                    Navigator.pushReplacement(
                                      ...
                                      MaterialPageRoute<void>(
                                        builder: (BuildContext context) => DetailAgenda(myId: listIdApps[index],),
                                      ),
                                    );
                                  },
                                  child: Column(
                                    children: [
                                      Container(
                                        ...
                                        height: ((MediaQuery.of(context).size.width - 60) * (9 / 16)) / 2,
                                      ...
                                      Container(
                                        ...
                                        child: Center(
                                          child: Text(
                                            listJudulApps[index].toString().substring(0,16),
