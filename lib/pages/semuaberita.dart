...
class SemuaBerita extends StatelessWidget {
  const SemuaBerita({super.key});
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void loadData() async {
    String tmpapp = await http.read(Uri.parse('https://simtaru.kaltimprov.go.id/api/posts?limit=20'));
    List<dynamic> tmpDataapp = jsonDecode(tmpapp).toList();
    setState(() {
      for (var element in tmpDataapp) {
        listIdApps.add(element['id']??'');
        listJudulApps.add(element['title']??'');
                                  ...
                                  onTap: () {
                                    Navigator.pushReplacement(
                                      ...
                                      MaterialPageRoute<void>(
                                        builder: (BuildContext context) => DetailBerita(myId: listIdApps[index],),
                                      ),
                                    );
                                  },
                                  child: Column(
                                    children: [
                                      Container(
                                        ...
                                        height: ((MediaQuery.of(context).size.width-40)) / 1,
